...
readline.question("Elige un dispositivo MIDI (introduce un número): ", (respuesta) => {
  ...
  wss.on('connection', (ws) => {
      ...
      ws.on('message', (message) => {
          ...
          if (data.ccNumber === 192 && typeof data.ccValue === 'number') {
              // Program Change
              midiOutput.send('program', {
                  number: data.ccValue,
                  channel: 0
              });
          } else if (data.ccNumber !== undefined && data.ccValue !== undefined) {
              // Control Change normal
              sendMidiCC(data.ccNumber, data.ccValue);
          } else if (data.note !== undefined && data.velocity !== undefined) {
              // Nota MIDI
              ...
              // Opcional: enviar noteoff después de 0.4s
              setTimeout(() => {
